(function () {
  const reportDataElement = document.getElementById('gco_report_data');
  ...
  const chartColors = ['#e04987', '#ff4621', '#78b89a', '#7c3bea', '#41b5e8'];
    ...
    new Chart(document.getElementById(canvasId), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: [chartColors[1], chartColors[0], chartColors[3]],
          borderWidth: 0
        }]
      },
      options: {
  ...
  function createYearLevelDonut(canvasId, level, index) {
    ...
    new Chart(document.getElementById(canvasId), {
      ...
      data: {
        ...
        datasets: [{
          data: [level.total, reportData.reportTotal - level.total],
          backgroundColor: [chartColors[index], remainingColors[index]],
          borderWidth: 0
        }]
      },
      options: {
  ...
  createBarChart('gco_service_type_chart', reportData.serviceTypes, false);
  createBarChart('gco_specialist_chart', reportData.specialists, true);

  reportData.yearLevels.forEach(function (level, index) {
    createYearLevelDonut('gco_year_level_chart_' + index, level, index);
  });

  createPieChart('gco_term_chart', reportData.terms, '65%');
  createPieChart('gco_school_year_chart', reportData.schoolYears, 0);
